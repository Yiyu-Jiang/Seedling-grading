...
def graph_rule_merge(instances, instance_masks, hole_assign_rows, adj, dmat):
    ...
    for i, j in zip(ii.tolist(), jj.tolist()):
        ...
        # 方案 D：一个有孔位分配，一个没有，但距离近且有 mask 重叠 → 合并
        if not should_merge and (hi is None) != (hj is None):
            if dij <= SAME_HOLE_CENTER_DIST_MAX and (iou > 0 or dil_iou >= OVERLAP_DILATED_IOU_MIN):
                should_merge = True
                reason.append("one_assigned_one_not_but_close_overlap")
...
def greedy_match_optimized_instances_with_mask_overlap(
    ...
    # ── 方案 B 兜底：将未匹配的实例分配到最近的孔位（允许多对一） ──
    matched_instance_ids = used_instances
    hole_pts_arr = np.array([h["center"] for h in hole_centers], dtype=np.float64)

    for ii, ins in enumerate(opt_instances):
        if ii in matched_instance_ids:
            continue
        ins_c = np.array(ins["centroid"], dtype=np.float64)
        dists = np.linalg.norm(hole_pts_arr - ins_c[None, :], axis=1)
        nearest_hi = int(np.argmin(dists))
        nearest_dist = float(dists[nearest_hi])
        # 放宽阈值：距离在 base_radius * 1.5 内才兜底分配
        if nearest_dist > base_radius * 1.5:
            continue
        h = hole_centers[nearest_hi]
        matches.append({
            "hole_index": int(h["hole_index"]),
            "row": int(h["row"]),
            "col": int(h["col"]),
...
def overlay_instances_on_image(image, opt_instances, alpha: float = 0.35):
    """
    将所有合并后的实例 mask 逐个叠加到原图上。

    修复点：
    1) 不再先生成单通道 label_mask 后统一着色，避免重叠区域中后写入 ID 覆盖先写入 ID；
    2) 不再只渲染 matched/assigned_instances，所有 opt_instances 均参与渲染，防止边缘行或未匹配实例被漏画；
    3) 对每个实例单独 alpha blend，并绘制轮廓，保证即使 polygon/mask 有 overlap，也能看到各实例边界。
    """
    ...
    for ins in opt_instances:
        ...
        cv2.drawContours(vis, contours, -1, color_bgr, thickness=1, lineType=cv2.LINE_AA)

    return vis


def scale_bbox_xyxy(bbox, scale):
    x1, y1, x2, y2 = bbox
    return (
...
def draw_tray_holes_presence_figure(image, hole_centers, matches, save_path: Path, opt_instances=None):
    """
    图1：整盘孔穴位置 + 有苗/无苗。

    修复点：
    - 在孔位圆圈之前，先叠加所有合并后的实例 mask；
    - 不依赖 matched 结果决定是否渲染实例 mask，因此底部行即使匹配失败也不会在图像中丢失；
    - 保持原有文件命名格式不变。
    """
    if opt_instances is not None and len(opt_instances) > 0:
        base = overlay_instances_on_image(image, opt_instances, alpha=0.35)
    else:
        base = image.copy()

    vis, scale = resize_long_side(base, VIS_LONG_SIDE)

    matched = {int(m["hole_index"]) for m in matches}
    for h in hole_centers:
        c = scale_point(h["center"], scale)
        color = GREEN if int(h["hole_index"]) in matched else GRAY
        cv2.circle(vis, c, 7, color, thickness=2, lineType=cv2.LINE_AA)

    cv2.putText(vis, "GREEN=seedling, GRAY=empty", (20, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.9, WHITE, 3, cv2.LINE_AA)
    cv2.putText(vis, "GREEN=seedling, GRAY=empty", (20, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.9, BLACK, 1, cv2.LINE_AA)
...
def draw_ranked_instances_area_boxes_and_empty_holes_figure(
    ...
    """
    图2：合并后实例 mask + 面积排序框 + 空穴标记。

    修复点：
    1) 所有 opt_instances 都参与 mask 渲染，不再只渲染 matched/assigned_instances；
    2) 不再用 label_mask[mask > 0] = rank 的单标签覆盖方式，避免 overlap 区域后写入覆盖先写入；
    3) 不再用面积分位数过滤实例 mask，避免底部行或小苗被误过滤；
    4) top/bottom 5% 仍按面积排序绘制 bbox，但不影响 mask 本体显示。
    """
    if len(opt_instances) == 0:
    ...
    # 关键修复：所有合并实例均渲染，不能只渲染 matched 或过滤后的 assigned 实例
    overlay = overlay_instances_on_image(image, opt_instances, alpha=alpha)
    vis, scale = resize_long_side(overlay, VIS_LONG_SIDE)

    # 面积排序仅用于 bbox 标注，不用于过滤 mask 渲染
    valid_instances = [ins for ins in opt_instances if float(ins.get("area", 0)) > 0]
    assigned_sorted = sorted(valid_instances, key=lambda x: float(x["area"]), reverse=True)

    n = len(assigned_sorted)
    k = max(1, int(np.ceil(n * 0.05))) if n > 0 else 0
    top_ids = {int(ins["opt_instance_index"]) for ins in assigned_sorted[:k]}
    bottom_ids = {int(ins["opt_instance_index"]) for ins in assigned_sorted[-k:]} if k > 0 else set()
    ...
    for ins in opt_instances:
        ...
        p1, p2 = scale_bbox_xyxy((x1, y1, x2, y2), scale)
    ...
    cv2.putText(vis, "Top5%=SKY, Bottom5%=RED, EmptyHole=BLUE", (20, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.9, WHITE, 3, cv2.LINE_AA)
    cv2.putText(vis, "Top5%=SKY, Bottom5%=RED, EmptyHole=BLUE", (20, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.9, BLACK, 1, cv2.LINE_AA)
    cv2.imwrite(str(save_path), vis)
...
def process_one_sample(json_path: Path, output_dir: Path, device: torch.device):
    ...
    draw_tray_holes_presence_figure(image=image, hole_centers=hole_centers, matches=matches, save_path=tray_presence_path, opt_instances=opt_instances)
